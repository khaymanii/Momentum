...
export async function GET() {
  try {
    console.log("1. Starting projects GET");

    const user = await requireUser();
    console.log("2. User:", user.uid);

    const database = db();
    console.log("3. Firestore initialized");
    ...
    console.log("4. Projects found:", snapshot.size);
...
export async function POST(request: NextRequest) {
  try {
    ...
    const project = {
      ...
      type: ["startup", "community", "product"].includes(body.type)
        ? body.type
        : "startup",
    ...
    return NextResponse.json(
      { project: { id: ref.id, ...project, createdAt: null, updatedAt: null } },
      { status: 201 },
    );
  } catch (error) {
    return apiError(error);
